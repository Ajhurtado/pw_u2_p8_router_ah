<template>
  <div>
    <nav>
    <!--<router-link to="/estudiante">Estudiante</router-link> |
    <router-link to="/contador/numeros">Contador</router-link> |
    <router-link to="/estudianteTabla">EstudianteTabla</router-link> |
    <router-link to="/pregunta">Pregunta</router-link> |
    <router-link to="/videojuego">Videojuego</router-link> |
    <router-link to="/animalesExtintos">AnimalesExtintos</router-link> |
    <router-link to="/responsivo">Responsivo</router-link>-->
    <router-link to="/estudianteMatricula">Estudiante Matricula</router-link> |
    <router-link to="/estudianteMatriculaFormulario">Estudiante Matricula Formulario</router-link>
  </nav>
  </div>
</template>

<script>
export default {

}
</script>

<style>

</style>